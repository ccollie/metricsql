...
// https://github.com/cube-js/cube/blob/master/rust/cubesql/cubesql/src/sql/session.rs
...
use crate::{
    ...
    sql::{
        database_variables::{
            DatabaseVariable, DatabaseVariablesToUpdate, mysql_default_session_variables,
            postgres_default_session_variables,
        },
...
impl SessionState {
    ...
    pub fn set_user(&self, user: Option<String>) {
        let mut guard = self
            .properties
            .write()
            .expect("failed to unlock properties for writing user");
        guard.user = user;
    }
