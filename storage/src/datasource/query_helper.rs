...
impl SqlDataSource {
    ...
    fn extract_metrics(&self, field_matchers: &[LabelFilter]) -> Result<Vec<LabelFilter>> {
        ...
        if let Some(field_matchers) = filters {
            ...
            let col_set = table_names.iter().collect::<HashSet<_>>();
            // opt-in set
            let mut result_set = HashSet::new();
            // opt-out set
            let mut reverse_set = HashSet::new();
    ...
    async fn selector_to_series_normalize_plan(
        ...
        // make filter exprs
        let mut scan_filters = self.matchers_to_expr(label_matchers.as_slice())?;
        ...
        let accurate_filters = self.matchers_to_expr(label_matchers.as_slice())?;
