...
use crate::{marshal_int64_array, unmarshal_int64_array, MarshalType};
...
pub fn ensure_marshal_unmarshal_int64_array(
    ...
    match mt {
        Lz4NearestDelta | Lz4NearestDelta2 | NearestDelta | NearestDelta2 => {
    ...
    let (mt_new, first_value_new) =
        marshal_int64_array(&mut b_new, va, precision_bits).expect("marshal_int64_array");
    assert_eq!(
    ...
    assert_eq!(
        mt_new, mt,
        "unexpected mt for prefixed va={:?}, precision_bits={}; got {}; expecting {}",
...
pub fn ensure_marshal_int64_array_size(
    va: &[i64],
    precision_bits: u8,
    ...
    if b.len() > max_size_expected {
        panic!(
            "too big size for marshaled {} items with precision_bits {}: got {}; expecting {}",
            va.len(),
            precision_bits,
            b.len(),
            max_size_expected
    ...
    if b.len() < min_size_expected {
        panic!(
            "too small size for marshaled {} items with precision_bits {}: got {}; expecting {}",
            va.len(),
            precision_bits,
            b.len(),
            min_size_expected
