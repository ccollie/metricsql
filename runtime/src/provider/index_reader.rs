...
pub enum IndexReaderError {
    ...
    #[error("Missing or empty matcher")]
    MissingMatcher,
...
pub type IndexReaderResult<T=()> = Result<T, IndexReaderError>;

/// IndexReader provides reading access of serialized index data.
pub trait IndexReader {
    async fn all_postings(&self) -> IndexReaderResult<impl PostingsList>;
    
    /// `sorted_label_values` returns sorted possible label values.
    async fn sorted_label_values(&self, name: &str, matchers: &[Matchers]) -> IndexReaderResult<Vec<String>>;

    /// `label_values` returns possible label values which may not be sorted.
    async fn label_values(&self, name: &str, matchers: &[Matcher]) -> IndexReaderResult<Vec<String>>;
    ...
    async fn postings(&self, name: &str, values: &[String]) -> IndexReaderResult<impl PostingsList>;
    ...
    async fn postings_for_label_matching(&self, name: &str, match_fn: impl Fn(&str) -> bool) -> IndexReaderResult<impl PostingsList>;
    ...
    async fn postings_for_all_label_values(&self, name: &str) -> IndexReaderResult<impl PostingsList>;

    /// `sorted_postings` returns a postings list that is reordered to be sorted by the label set of the underlying series.
    async fn sorted_postings(&self, postings: impl Iterator<Item=SeriesRef>) -> IndexReaderResult<impl PostingsList>;
    ...
    async fn label_names(&self, matchers: &[Matcher]) -> IndexReaderResult<Vec<String>>;

    /// `label_value_for` returns the label value for the given label name in the series referred to by ID.
    /// If the series couldn't be found or the series doesn't have the requested label a
    /// storage.ErrNotFound is returned as error.
    async fn label_value_for(&self, id: SeriesRef, label: &str) -> IndexReaderResult<String>;

    /// `label_names_for` returns all the label names for the series referred to by the postings.
    /// The names returned are sorted.
    async fn label_names_for(&self, postings: impl Iterator<Item=SeriesRef>) -> IndexReaderResult<Vec<String>>;
}