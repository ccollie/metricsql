...
pub fn test_rows_equal(values: &[f64], timestamps: &[i64], values_expected: &[f64], timestamps_expected: &[i64]) {
    ...
    assert_eq!(values.len(), timestamps.len(),
               "values.len() doesn't match timestamps.len(); got {} vs {}", values.len(), timestamps.len());
...
pub fn compare_values(vs1: &[f64], vs2: &[f64]) -> RuntimeResult<()> {
    assert_eq!(vs1.len(), vs2.len(),
               "unexpected number of values; got {}; want {}", vs1.len(), vs2.len());
    for (i, v1) in vs1.iter().enumerate() {
        let v2 = &vs2[i];
        assert!(compare_floats(*v2, *v1), "unexpected value; got {}; want {}", v1, v2);
    }
    Ok(())
}
