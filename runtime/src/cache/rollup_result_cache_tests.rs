...
mod tests {
    ...
    fn setup() -> TestContext {
        ...
        let fe = FunctionExpr::from_single_arg("foo", Expression::MetricExpression(me)).unwrap();

        let mut ae = AggregationExpr::new(&AggregateFunction::Sum);
        ae.args.push(Box::new(Expression::Function(fe.clone())));

        let cache = RollupResultCache::default();

        TestContext {
            ec,
            ae: Expression::Aggregation(ae),
            fe: Expression::Function(fe),
            cache,
            window,
        }
    }
