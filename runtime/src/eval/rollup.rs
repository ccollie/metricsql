...
use crate::{get_timeseries, get_timestamps, EvalConfig, MetricName};
...
impl RollupEvaluator {
    ...
    pub fn from_function(expr: &FunctionExpr) -> RuntimeResult<Self> {
        ...
        let mut res = Self::create_internal(func, &re, Expression::Function(expr.clone()), args)?;
    ...
    fn eval_with_metric_expr(
        ...
        let tss = match &self.expr {
            Expression::Aggregation(ae) => self.eval_with_incremental_aggregate(
                &ae,
                &mut rss,
                rcs,
                pre_func,
...
fn get_rollup_expr_arg(arg: &Expression) -> RuntimeResult<RollupExpr> {
    ...
    return match re.expr.deref() {
        Expression::MetricExpression(me) => {
            ...
            let arg = Expression::Rollup(RollupExpr::new(Expression::MetricExpression(me.clone())));
