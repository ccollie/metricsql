...
impl Display for Expression {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Expression::String(s) => write!(f, "{}", enquote::enquote('"', &*self.s))?,
...
impl RollupExpr {
    ...
    pub fn set_offset(mut self, expr: i64) -> Self {
        self.offset = Some(expr);
        self
    }

    pub fn set_window(mut self, expr: i64) -> Self {
...
impl MetricExpr {
    pub fn new<S: Into<String>>(group_name: S, label_filters: Vec<LabelFilter>) -> MetricExpr {
        MetricExpr {
            label_filters,
...
impl Display for MetricExpr {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        if self.metric_group.len() > 0 {
            write!(f, "{}", escape_ident(&self.metric_group))?;
        }
