...
fn optimize_in_place(mut e: &Expression)  {
    ...
    fn optimize_boxed(expr: BExpression) {
        optimize_in_place(&*expr)
    }
    ...
    match e {
        ...
        BinaryOperator(op) => {
            ...
            let lfs = get_common_label_filters(op);
            if lfs.len() > 0 {
                pushdown_binary_op_filters_in_place(op, lfs);
            }
        },
        _ => {
            // do nothing
                    ...
                    if e.join_modified.is_some() {
                        return match e.join_modifier.op {
                            ...
                            JoinOp::Right => {
                                ...
                                let lfsLeft = trim_filters_by_group_modifier(lfs_left, e);
                                union_label_filters(lfsLeft, lfs_right)
                            },
                            _ => {
                                // {f1} * {f2} -> {f1, f2}
                                // {f1} * on() {f2} -> {}
...
fn pushdown_binary_op_filters_in_place(mut e: &Expression, common_filters: &Vec<LabelFilter>) {
    use Expression::*;
    ...
    match e {
        ...
        Function(f) => {
            ...
            if arg.is_some() {
                pushdownBinaryOpFiltersInplace(arg.unwrap(), lfs);
            }
        },
        BinaryOperator(bo) => {
...
fn get_func_arg_for_optimization(funcName: &str, args: &Vec<Expression>) -> Option<Expression> {
    let idx = get_func_arg_idx_for_optimization(funcName, args);
    if idx < 0 || idx >= args.len() as i32 {
        return None;
    }
    return args[idx];
}

fn get_func_arg_idx_for_optimization(funcName: &str, args: &Vec<Expression>) -> usize {
    let lower = funcName.to_lowercase().as_str();
    if is_rollup_func(lower) {
        return get_rollup_arg_idx_for_optimization(funcName, args);
    }
    if is_transform_func(funcName) {
        return get_transform_arg_idx_for_optimization(funcName, args);
    }
    if is_aggr_func(funcName) {
        return get_aggr_arg_idx_for_optimization(funcName, args);
    }
    return -1;
}
