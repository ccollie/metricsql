...
use enquote::unquote;
use regex::Error;
use crate::{BExpression, BinaryOpExpr, Expression, MetricExpr, RollupExpr, StringExpr, WithArgExpr};
use super::lexer::*;
...
pub fn expand_string(mut e: &StringExpr) -> Result<StringExpr, Error> {
    if e.len() > 0 {
        // Already expanded.
        return Ok(e);
    }
...
fn expand_modifier_args(mut was: &Vec<WithArgExpr>, args: Vec<String>) -> Result<Vec<String>, Error> {
    let mut dst_args: Vec<String> = Vec![];
...
fn expand_with_expr_ext(was: &Vec<WithArgExpr>, wa: &WithArgExpr, args: &Vec<Expression>) -> Vec<Expression> {
    match expr {
