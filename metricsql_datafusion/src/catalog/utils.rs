...
use crate::catalog::consts::DEFAULT_CATALOG_NAME;
...
pub struct UInt32VectorBuilder(Vec<Option<u32>>);

impl UInt32VectorBuilder {
    pub(crate) fn with_capacity(capacity: usize) -> Self {
        Self(Vec::with_capacity(capacity))
    }

    pub(crate) fn push(&mut self, value: Option<u32>) {
        self.0.push(value);
    }

    pub(crate) fn finish(self) -> UInt32Array {
        UInt32Array::from(self.0)
    }
}

pub struct StringVectorBuilder(Vec<Option<String>>);
...
impl StringVectorBuilder {
    ...
    pub(crate) fn push(&mut self, value: Option<&str>) {
        self.0.push(value.map(ToOwned::to_owned));
    }

    pub(crate) fn finish(self) -> StringArray {
        StringArray::from(self.0)
    }
}
