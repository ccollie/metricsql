...
use crate::catalog::{build_db_string, CatalogManager, DeregisterTableRequest, RegisterSchemaRequest, RegisterTableRequest};
use crate::catalog::consts::{DEFAULT_CATALOG_NAME, DEFAULT_PRIVATE_SCHEMA_NAME, DEFAULT_SCHEMA_NAME, INFORMATION_SCHEMA_NAME};
...
use crate::catalog::metrics::{METRIC_CATALOG_MANAGER_CATALOG_COUNT, METRIC_CATALOG_MANAGER_SCHEMA_COUNT, METRIC_CATALOG_MANAGER_TABLE_COUNT};
use crate::table::TableRef;

type Result<T> = std::result::Result<T, crate::catalog::error::Error>;
    ...
    pub fn register_catalog_sync(&self, name: &str) -> Result<bool> {
        ...
        match catalogs.entry(name.clone()) {
            Entry::Vacant(e) => {
                ...
                e.insert(catalog);
                METRIC_CATALOG_MANAGER_CATALOG_COUNT.inc();
                Ok(true)
            }
    ...
    pub fn register_schema_sync(&self, request: RegisterSchemaRequest) -> Result<bool> {
        ...
        match catalog.entry(request.schema) {
            Entry::Vacant(e) => {
                ...
                METRIC_CATALOG_MANAGER_SCHEMA_COUNT.inc();
                Ok(true)
            }
            Entry::Occupied(_) => Ok(false),
        }
...
mod tests {
    ...
    use crate::catalog::DeregisterTableRequest;
