...
mod test {
    ...
    use crate::object_store::services::{Fs, S3};
    use crate::object_store::test_util::TempFolder;
    use crate::object_store::{ObjectStore, ObjectStoreBuilder};
    ...
    async fn test_azblob_backend() -> Result<()> {
        ...
        if let Ok(container) = env::var("GT_AZBLOB_CONTAINER") {
            if !container.is_empty() {
                tracing::info!("Running azblob test.");
