...
impl PhysicalPlanAdapter {
    pub fn new(schema: SchemaRef, df_plan: Arc<dyn ExecutionPlan>) -> Self {
        Self {
            schema,
            df_plan,
...
impl ExecutionPlan for DfPhysicalPlanAdapter {
    fn as_any(&self) -> &dyn Any {
        self
    }
        ...
        fn execute(
            ...
            let recordbatches = RecordBatches::try_new(
                ...
                vec![
                    RecordBatch::new(
                        schema.clone(),
                        vec![Arc::new(Int32Array::from_slice(vec![1])) as _],
                    )
                    .unwrap(),
                    RecordBatch::new(
                        schema,
                        vec![Arc::new(Int32Array::from_slice(vec![2, 3])) as _],
                    )
                    .unwrap(),
                ],
            )
...
+---+"#
    ...
    fn test_physical_plan_adapter() {
        let df_schema = Arc::new(Schema::new(vec![Field::new(
            "name",
            DataType::Utf8,
            true,
        )]));
