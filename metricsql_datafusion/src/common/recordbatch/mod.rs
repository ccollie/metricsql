...
pub use arrow::record_batch::RecordBatch as DfRecordBatch;
...
use crate::common::recordbatch::error::Result;
...
pub type SendableRecordBatchStream = Pin<Box<dyn RecordBatchStream<Item=Result<RecordBatch>> + Send>>;
...
impl RecordBatches {
    pub fn try_from_columns<I: IntoIterator<Item = ArrayRef>>(
        schema: SchemaRef,
        columns: I,
    ) -> Result<Self> {
...
mod tests {
    ...
    fn test_recordbatches_try_from_columns() {
        ...
        let result = RecordBatches::try_from_columns(
            schema.clone(),
            vec![Arc::new(StringArray::from(vec!["hello", "world"])) as _],
        );
        assert!(result.is_err());

        let v: ArrayRef = Arc::new(Int32Array::from_slice([1, 2]));
    ...
    fn test_recordbatches_try_new() {
        ...
        let column_b = Field::new("b", DataType::Utf8, false);
        let column_c = Field::new("c", DataType::Boolean, false);

        let va: ArrayRef = Arc::new(Int32Array::from_slice([1, 2]));
        let vb: ArrayRef = Arc::new(StringArray::from(vec!["hello", "world"]));
        let vc: ArrayRef = Arc::new(BooleanArray::from(vec![true, false]));
...
+---+-------+";
    ...
    async fn test_simple_recordbatch_stream() {
        ...
        let va1: ArrayRef = Arc::new(Int32Array::from_slice([1, 2]));
        let vb1: ArrayRef = Arc::new(StringArray::from(vec!["a", "b"]));
        let batch1 = RecordBatch::new(schema.clone(), vec![va1, vb1]).unwrap();

        let va2: ArrayRef = Arc::new(Int32Array::from_slice([3, 4, 5]));
        let vb2: ArrayRef = Arc::new(StringArray::from(vec!["c", "d", "e"]));
