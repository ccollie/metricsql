...
impl Stream for DfRecordBatchStreamAdapter {
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        match Pin::new(&mut self.stream).poll_next(cx) {
            ...
            Poll::Ready(Some(record_batch)) => match record_batch {
                Ok(batch) => Poll::Ready(Some(Ok(batch.into_df_record_batch()))),
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        match Pin::new(&mut self.stream).poll_next(cx) {
            ...
            Poll::Ready(Some(df_record_batch)) => {
                let df_record_batch = df_record_batch.context(PollStreamSnafu)?;
...
mod test {
    ...
    async fn test_async_recordbatch_stream_adaptor() {
        ...
        let batch1 = RecordBatch::new(
            schema.clone(),
            vec![Arc::new(Int32Array::from_slice([1])) as _],
        )
        .unwrap();
        let batch2 = RecordBatch::new(
            schema.clone(),
            vec![Arc::new(Int32Array::from_slice([2])) as _],
        )
        .unwrap();
        ...
        let err = RecordBatches::try_collect(Box::pin(adapter))
            ...
            .unwrap_err();
        assert!(
            matches!(err, ExternalSnafu { .. }),
            "unexpected err {err}"
        );

        let failed_to_init_stream =
            new_future_stream(Err(ExternalSnafu
