...
use crate::datasource::file_format::csv::{CsvConfigBuilder, CsvFormat, CsvOpener};
use crate::datasource::file_format::Format;
use crate::datasource::file_format::json::{JsonFormat, JsonOpener};
use crate::datasource::file_format::orc::{OrcFormat, OrcOpener};
use crate::datasource::file_format::parquet::{DefaultParquetFileReaderFactory, ParquetFormat};
use crate::file_engine::error::{
    BuildCsvConfigSnafu, BuildStreamAdapterSnafu, BuildStreamSnafu, ParquetScanPlanSnafu, ProjectArrowSchemaSnafu,
    Result
};
...
fn build_orc_opener(output_schema: Arc<Schema>, config: &ScanPlanConfig) -> Result<OrcOpener> {
    Ok(OrcOpener::new(
        config.store.clone(),
        output_schema,
...
fn build_record_batch_stream<T: FileOpener + Send + 'static>(
    ...
    let adapter = RecordBatchStreamAdapter::try_new(Box::pin(stream))
        .context(BuildStreamAdapterSnafu)?;
    Ok(Box::pin(adapter))
}
...
fn new_parquet_stream_with_exec_plan(
    ...
    Ok(Box::pin(
        RecordBatchStreamAdapter::try_new(stream).context(BuildStreamAdapterSnafu)?,
    ))
}
