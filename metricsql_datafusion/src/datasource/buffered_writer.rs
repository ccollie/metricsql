    ...
    > LazyBufferedWriter<T, U, F>
{
    ...
    pub async fn close_with_arrow_writer(mut self) -> Result<(FileMetaData, u64)> {
        let encoder = self.encoder.take().context(BufferedWriterClosedSnafu)?;
    ...
    > LazyBufferedWriter<T, U, F>
{
    ...
    pub async fn close_inner_writer(&mut self) -> Result<()> {
        if let Some(writer) = &mut self.writer {
            writer.shutdown().await.context(AsyncWriteSnafu)?;
        }
        Ok(())
    }
    ...
    pub async fn write(&mut self, batch: &RecordBatch) -> Result<()> {
        let encoder = self.encoder.as_mut().context(BufferedWriterClosedSnafu)?;
        encoder.write(batch)?;
