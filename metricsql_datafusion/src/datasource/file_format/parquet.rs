...
use crate::datasource::buffered_writer::{
    ArrowWriterCloser, DfRecordBatchEncoder, LazyBufferedWriter,
};
use crate::datasource::error::{
    EncodeRecordBatchSnafu, ParquetToSchemaSnafu, ReadObjectSnafu, ReadParquetSnafuSnafu,
    ReadRecordBatchSnafu, Result, WriteObjectSnafu, WriteParquetSnafu,
};
use crate::datasource::share_buffer::SharedBuffer;
use crate::object_store;
use crate::object_store::{ObjectStore, Reader};
...
impl FileFormat for ParquetFormat {
    async fn infer_schema(&self, store: &ObjectStore, path: &str) -> Result<Schema> {
        let mut reader = store.reader(path).await.context(ReadObjectSnafu { path })?;

        let metadata = reader.get_metadata().await.context(ReadParquetSnafuSnafu)?;
...
impl BufferedWriter {
        ...
        Ok(Self {
            inner: LazyBufferedWriter::new(
                ...
                Box::new(move |path| {
                    ...
                    Box::pin(
                        async move { store.writer(&path).await.context(WriteObjectSnafu { path }) },
                    )
                }),
            ),
        })
    ...
    pub async fn write(&mut self, arrow_batch: &RecordBatch) -> Result<()> {
...
pub async fn stream_to_parquet(
    ...
    while let Some(batch) = stream.next().await {
        let batch = batch.context(ReadRecordBatchSnafu)?;
...
#[cfg(test)]
mod tests {
    use crate::datasource::test_util::{format_schema, test_store};
