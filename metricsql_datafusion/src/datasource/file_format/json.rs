...
use crate::datasource::error::{
    InferSchemaSnafu, JoinHandleSnafu, ParseFormatSnafu,
    ReadObjectSnafu, WriteRecordBatchSnafu
};
use crate::datasource::error::{self, Result};
use crate::datasource::file_format;
use crate::datasource::share_buffer::SharedBuffer;
use crate::object_store::ObjectStore;

use super::{FileFormat, open_with_decoder, stream_to_file};
...
impl FileFormat for JsonFormat {
    async fn infer_schema(&self, store: &ObjectStore, path: &str) -> Result<Schema> {
        ...
        common_runtime::spawn_blocking_read(move || {
            ...
            let schema = infer_json_schema_from_iterator(iter).context(InferSchemaSnafu)?;

            Ok(schema)
        })
        .await
        .context(JoinHandleSnafu)?
    }
}
...
impl DfRecordBatchEncoder for json::Writer<SharedBuffer, LineDelimited> {
    fn write(&mut self, batch: &RecordBatch) -> Result<()> {
        self.write(batch).context(WriteRecordBatchSnafu)
    }
}

#[cfg(test)]
mod tests {
    use crate::datasource::file_format::{FORMAT_COMPRESSION_TYPE, FORMAT_SCHEMA_INFER_MAX_RECORD};
