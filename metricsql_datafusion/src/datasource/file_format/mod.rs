...
use crate::datasource::error::{
    self,
    MergeSchemaSnafu, ReadRecordBatchSnafu, Result, UnsupportedFormatSnafu, WriteObjectSnafu
};
use crate::object_store::ObjectStore;

use super::buffered_writer::{DfRecordBatchEncoder, LazyBufferedWriter};
...
impl TryFrom<&HashMap<String, String>> for Format {
    ...
    fn try_from(options: &HashMap<String, String>) -> Result<Self> {
        ...
        match format.as_str() {
            ...
            _ => UnsupportedFormatSnafu { format: &format }.fail(),
        }
    }
}
...
pub async fn infer_schemas(
    ...
    ArrowSchema::try_merge(schemas).context(MergeSchemaSnafu)
}
