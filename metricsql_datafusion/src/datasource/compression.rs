...
use crate::datasource::error::UnsupportedCompressionTypeSnafu;
...
impl FromStr for CompressionType {
    ...
    fn from_str(s: &str) -> Result<Self> {
        match s {
            v if v.eq_ignore_ascii_case("gzip") => Ok(Self::Gzip),
            v if v.eq_ignore_ascii_case("gz") => Ok(Self::Gzip),
            v if v.eq_ignore_ascii_case("bzip2") => Ok(Self::Bzip2),
            v if v.eq_ignore_ascii_case("bz2") => Ok(Self::Bzip2),
            v if v.eq_ignore_ascii_case("xz") => Ok(Self::Xz),
            v if v.eq_ignore_ascii_case("zstd") => Ok(Self::Zstd),
            v if v.eq_ignore_ascii_case("zst") => Ok(Self::Zstd),
            "" => Ok(Self::Uncompressed),
            _ => UnsupportedCompressionTypeSnafu {
                compression_type: s,
            }
                .fail(),
        }
