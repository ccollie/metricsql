...
use crate::datasource::error::{EmptyHostPathSnafu, InvalidUrlSnafu, Result, UnsupportedBackendProtocolSnafu};
use crate::object_store::ObjectStore;

use super::util::find_dir_and_filename;
...
pub fn parse_url(url: &str) -> Result<(String, Option<String>, String)> {
    ...
    match parsed_url {
        ...
        Err(err) => Err(err).context(InvalidUrlSnafu { url }),
    }
}
...
pub fn build_backend(url: &str, connection: &HashMap<String, String>) -> Result<ObjectStore> {
    ...
    match schema.to_uppercase().as_str() {
        S3_SCHEMA => {
            let host = host.context(EmptyHostPathSnafu {
                url: url.to_string(),
            })?;
        ...
        _ => UnsupportedBackendProtocolSnafu {
            protocol: schema,
            url,
        }
        .fail(),
