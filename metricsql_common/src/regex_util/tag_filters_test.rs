#[cfg(test)]
mod tests {
    use crate::regex_util::tag_filter::{
        get_regexp_from_cache, simplify_regexp, TagFilter, TagFilters,
    };
    ...
    fn test_get_regexp_from_cache() {
        fn f(
            s: &str,
            or_values_expected: &[&str],
            expected_matches: &[&str],
            expected_mismatches: &[&str],
            suffix_expected: &str,
        ) {
            for _ in 0..3 {
                ...
                assert_eq!(
                    rcv.or_values, or_values_expected,
                    "unexpected orValues for s={:?}; got {:?}; want {:?}",
                    s, rcv.or_values, or_values_expected
                );
                assert_eq!(
                    rcv.literal_suffix, suffix_expected,
                    "unexpected literalSuffix for s={:?}; got {}; want {suffix_expected}",
                    s, rcv.literal_suffix
                );
                for expected_match in expected_matches.iter() {
                    assert!(
                        rcv.re_match.matches(expected_match),
                        "s={:?} must match {expected_match}",
                        s
                    );
                }
                for expected_mismatch in expected_mismatches {
                    assert!(
                        !rcv.re_match.matches(expected_mismatch),
                        "s={:?} must mismatch {:?}",
                        s,
                        expected_mismatches
                    );
    ...
    fn init_tf(
        ...
        assert_eq!(
            expected_prefix, tf.prefix,
            "unexpected tf.prefix; got {}; want {expected_prefix}",
            tf.prefix
        );
        tf
    }
    ...
    fn regexp_or_suffixes() {
        ...
        assert_eq!(
            tf.or_suffixes,
            &["bar", "foo"],
            "unexpected or_suffixes; got {:?}; want &[\"bar\", \"foo\"]",
            tf.or_suffixes
        );
    ...
    fn test_simplify_regexp() {
        fn f(s: &str, expected_prefix: &str, expected_suffix: &str) {
            ...
            assert_eq!(
                ...
                "unexpected prefix for s={:?}; got {prefix}; want {:?}",
                s, &expected_prefix
            );
            assert_eq!(
                suffix, expected_suffix,
                "unexpected suffix for s={:?}; got {suffix}; want {:?}",
                s, expected_suffix
            );
    ...
    fn test_tag_filters_string() {
        ...
        let must_add =
            |tfs: &mut TagFilters, key: &str, value: &str, is_negative: bool, is_regexp: bool| {
        ...
        assert_eq!(
            s, expected,
            "unexpected TagFilters.to_string(); got {s}; want {expected}"
        )
    }
    ...
    fn test_tag_filters_add_empty() {
        ...
        let expect_tag_filter =
            |tfs: &TagFilters, idx: usize, value: &str, is_negative: bool, is_regexp: bool| {
                if idx >= tfs.len() {
                    panic!(
                        "missing tag filter #{}; len(tfs)={}, tfs={:?}",
                        idx,
                        tfs.len(),
                        tfs
                    )
                ...
                assert_eq!(
                    tf.value, value,
                    "unexpected tag filter value; got {}; want {value}",
                    tf.value
                );
                assert_eq!(
                    tf.is_negative, is_negative,
                    "unexpected tag filter is_negative; got {}; want {is_negative}",
                    tf.is_negative
                );
                assert_eq!(
                    tf.is_regexp, is_regexp,
                    "unexpected is_regexp; got {}; want {}",
                    tf.is_regexp, is_regexp
                )
            };
        ...
        if tfs.len() != 0 {
            panic!(
                "unexpectedly added empty regexp filter {:?}",
                tfs.get(0).unwrap()
            );
        }
