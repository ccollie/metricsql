#[cfg(test)]
mod tests {
    use super::{get_common_label_filters, pushdown_binary_op_filters};
    use crate::ast::Expression::MetricExpression;
    use crate::ast::MetricExpr;
    use crate::hir::push_down_filters::{get_common_label_filters, pushdown_binary_op_filters};
    ...
    fn test_pushdown_binary_op_filters() {
        let f = |q: &str, filters: &str, result_expected: &str| {
            ...
            let filters_expr =
                parse(filters).expect(format!("cannot parse filters {}", filters).as_str());
            match filters_expr {
    ...
    fn optimize_common_binary_expressions() {
        ...
        validate_optimized(
            r#"f(foo, bar{baz=~"sdf"} + aa{baz=~"axx", aa="b"})"#,
            r#"f(foo, bar{aa="b", baz=~"axx", baz=~"sdf"} + aa{aa="b", baz=~"axx", baz=~"sdf"})"#,
        );
        validate_optimized(
