...
impl Lexer {
    ...
    pub fn next(&mut self) -> Result<(), Error> {
        ...
        if self.next_tokens.len() > 0 {
            self.token = self.next_tokens.pop().unwrap();
            return Ok(())
        }
    ...
    pub fn positive_number(&mut self) -> Result<f64, Error> {
        ...
        let (skip_chars, is_special, base) = scan_special_integer_prefix(n);
        if is_special {
            if skip_chars {
                n = &n[skip_chars..];
            }
    ...
    pub fn positive_duration(&mut self) -> Result<String, Error> {
        ...
        if is_positive_duration(s) {
            ...
            match self.positive_number() {
                Ok(res) => s = res.to_string(),
                Err(e) => Err(
...
fn scan_positive_number(s: &str) -> Result<&str, Error> {
    ...
    if s[i] == '.' {
        ...
        let mut j = i;
        let mut size = 0;
        for (k, ch) in s1.chars().enumerate() {
        ...
        if size == 0 {
            return Err(format!("missing fractional part in {}", s))
        }
        i = j;
        if i == len {
            return Ok(s)
    ...
    // Scan exponent part.
    if i == len(s) {
        return Ok(format!("missing exponent part in %q", s))
    }
    let mut s1 = &s[i..];
    ...
    if j == i {
        return Err(Error::from(format!("missing exponent part in {}", s)))
    }
    return s[:j]
}
