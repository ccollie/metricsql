...
mod tests {
	...
	fn test_parse_metric_expr() {
		...
		another(r#"{foo="bar"}[5m] oFFSEt 10y"#, r#"{foo="bar"}[5m] offset 10y"#);
		same("METRIC");
		same("metric");
	...
	fn test_parse_at_modifier() {
		...
		another(r#"foo{bar="baz"}[5m:4s] @ (end() - 3.5m) offset 2.4h"#, r#"foo{bar="baz"}[5m:4s] offset 2.4h @ (end() - 3.5m)"#);
		another(r#"foo @ start() + (bar offset 3m @ end()) / baz OFFSET -5m"#, r#"foo @ start() + (bar offset 3m @ end() / baz offset -5m)"#);
	...
	fn test_parse_identifiers_with_escape_chars() {
		...
		another(r#"\x2E\x2ef\oo{b\xEF\ar="aa"}"#, r#"\x2e.foo{b\xefar="aa"}"#);
	...
	//	same("m + ignoring (a, b) n[5m]");
		...
		another(r#"5 - 1 + 3 * 2 ^ 2 ^ 3 - 2  OR Metric {Bar= "Baz", aaa!="bb",cc=~"dd" ,zz !~"ff" } "#,
	...
	fn test_parse_with_expr() {
		...
		another(r#"with (foo = bar) {__name__=~"foo"}"#, r#"{__name__=~"foo"}"#);
		another(r#"with (foo = bar) foo{__name__="foo"}"#, "bar");
		another(r#"with (foo = bar) {__name__="foo", x="y"}"#, r#"bar{x="y"}"#);
		another(r#"with (foo(bar) = {__name__!="bar"}) foo(x)"#, r#"{__name__!="bar"}"#);
		another(r#"with (foo(bar) = bar{__name__="bar"}) foo(x)"#, "x");
		another(r#"with (foo\-bar(baz) = baz + baz) foo\-bar((x,y))"#, "(x, y) + (x, y)");
		another(r#"with (foo\-bar(baz) = baz + baz) foo\-bar(x*y)"#, r#"(x * y) + (x * y)"#);
		another(r#"with (foo\-bar(baz) = baz + baz) foo\-bar(x\*y)"#, r#"x\*y + x\*y"#);
		another(r#"with (foo\-bar(b\ az) = b\ az + b\ az) foo\-bar(x\*y)"#, r#"x\*y + x\*y"#);
	...
	fn invalid_metric_expr() {
		...
		assert_invalid("m{x=y+5}");
		assert_invalid("m keep_metric_names"); // keep_metric_names cannot be used with metric expression
	}
