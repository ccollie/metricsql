...
use crate::ast::{Expression, ExpressionNode, StringExpr};
use crate::common::{LabelFilter, LabelFilterOp, ReturnType};
...
impl MetricExpr {
    pub fn new<S: Into<String>>(name: S) -> MetricExpr {
        let name_filter = LabelFilterExpr::new(
            LabelFilterOp::Equal,
            "__name__",
            StringExpr::from(name.into()),
        )
        .unwrap();
        MetricExpr {
    ...
    pub fn is_expanded(&self) -> bool {
        self.label_filters.len() == 0 || self.label_filters.iter().all(|x| x.is_resolved())
    }
