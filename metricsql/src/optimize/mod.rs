...
// original source:
// https://github.com/apache/arrow-datafusion/blob/main/datafusion/optimizer/src/simplify_expressions/regex.rs
use crate::ast::Expr;
...
pub fn simplify_regex_expr(left: Box<Expr>, op: Operator, right: Box<Expr>) -> ParseResult<Expr> {
    ...
    if let Expr::Literal(ScalarValue::Utf8(Some(pattern))) = right.as_ref() {
        match regex_syntax::Parser::new().parse(pattern) {
            ...
            Err(e) => {
                ...
                return Err(ParseError::InvalidRegex(pattern.clone()));
            }
        }
    }
