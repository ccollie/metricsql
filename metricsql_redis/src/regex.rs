...
const MAX_REGEX_ALTERNATIONS_EXPANSION: usize = 16;

pub fn simplify_regex_expr(pattern: &str, op: LabelFilterOp, left: Box<Expr>) -> Result<Expr> {
...
/// extracts a string literal assuming that [`is_anchored_literal`]
/// returned true.
fn anchored_literal_to_expr(v: &[Hir]) -> Option<&str> {
    match v.len() {
        2 => Some(lit("")),
        3 => {
...
fn lower_simple(mode: &OperatorMode, left: &Expr, hir: &Hir) -> Option<Expr> {
    match hir.kind() {
        ...
        HirKind::Concat(inner) if is_anchored_literal(inner) => {
            return anchored_literal_to_expr(inner)
                .map(|right| mode.expr_matches_literal(Box::new(left.clone()), Box::new(right)));
        }
